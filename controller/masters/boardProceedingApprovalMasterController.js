import boardProceedingApproval from "../../db/models/boardproceedingapprovalmaster.js";
...
export const getAllBoardProceedingApproval = async (req, res) => {
    try {
      
      const newApp = await boardProceedingApproval.findAll({
      ...
      {
        await sendRes.sendResponse(res, sendRes.statusCode.OK, 1, sendRes.statusMessage.DATA_GET_FOUND,newApp);
      }
  ...
  export const createBoardProceedingApproval = async (req, res) => {
    try {
      const decryptedData = await encryptDecrypt.decrypt(req.body.body);
      const JsonData = JSON.parse(decryptedData);
      const existing = await boardProceedingApproval.findOne({
        where: { Approval: JsonData.Approval ,isDeleted:false,isActive:true},
      });
      ...
      const newApp = await boardProceedingApproval.create({
  ...
  export const getBoardProceedingApprovalById = async (req, res) => {
    try {
      const decryptedData = await encryptDecrypt.decrypt(req.query.Id);
      //console.log(decryptedData);
      const JsonData = JSON.parse(decryptedData);
      //console.log(JsonData)
      const newApp = await boardProceedingApproval.findOne({ 
        where: {
        id: JsonData,
        isDeleted:false,
        isActive:true
  ...
  export const updateBoardProceedingApproval = async (req, res) => {
    try {
      const decryptedData = await encryptDecrypt.decrypt(req.body.body);
      //console.log(decryptedData);
      const JsonData = JSON.parse(decryptedData);
     // console.log(JsonData)
      const newApp = await boardProceedingApproval.update(
        {
            Approval:JsonData.Approval,
      ...
      {
        await sendRes.sendResponse(res, sendRes.statusCode.OK, 1, sendRes.statusMessage.DATA_UPDATED,newApp);
      }
  ...
  export const deleteBoardProceedingApproval = async (req, res) => {
    try {
      const decryptedData = await encryptDecrypt.decrypt(req.query.Id);
      //console.log(decryptedData);
      const JsonData = JSON.parse(decryptedData);
     // console.log(JsonData)
      ...
      {
        await sendRes.sendResponse(res, sendRes.sendRes.statusCode.OK, 1, sendRes.statusMessage.DELETE_DATA,newApp);
      }
